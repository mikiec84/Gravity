...
class param_ : public constant_ {
 protected:
  NType _intype;
  shared_ptr<map<string, unsigned>> _indices =
      nullptr; /*<< A map storing all the indices this parameter has, the key is
                  represented by a string, while the entry indicates the right
                  position in the values and bounds vectors */
  shared_ptr<vector<string>> _rev_indices =
      nullptr; /*<< A vector storing all the indices this parameter has */

  /* (Guanglei) added this part to record the indices of sdp variables. SDP
   * should be indexed by a pair of integers. This is true for all SDP solvers.
   */
 ...
 public:
  ...
  shared_ptr<int>
      _vec_id; /**< index in the vector array (useful for Cplex). **/

  string _name;
  shared_ptr<vector<vector<unsigned>>> _ids =
      nullptr; /*<<A vector storing all the indices this parameter has in the
  ...
  bool _new = true;       /**< Will become false once this param is added to a
  ...
  size_t get_id_inst(unsigned inst1, unsigned inst2) const {
    ...
    return inst2;
    //        throw invalid_argument("Calling get_id_inst on a non indexed
    //        parpam\n");
  };
  ...
  pair<size_t, size_t> get_sdp_inst(unsigned inst = 0) const {
    ...
    //            res =
    //            make_pair(std::stoi(it->first.substr(0,1)),std::stoi(it->first.substr(2,1)));
  ...
  string get_name(size_t inst) const {
    ...
    if (_is_indexed &&
        name.find("[") != std::string::npos) {  // Name has index already
      return name;
    }
    if (_is_indexed) {
  ...
  shared_ptr<map<string, unsigned>> get_indices() const { return _indices; }

  shared_ptr<vector<string>> get_rev_indices() const { return _rev_indices; }
  ...
  shared_ptr<vector<vector<unsigned>>> get_ids() const { return _ids; }

  void set_type(NType type) { _intype = type; }

  /** Querries */

  bool is_binary() const { return (_intype == binary_); };

  bool is_integer() const { return (_intype == integer_); };

  bool is_float() const { return (_intype == float_); };

  bool is_double() const { return (_intype == double_); };

  bool is_long() const { return (_intype == long_); };

  Sign get_all_sign()
      const; /**< If all instances of the current parameter/variable have the
                same sign, it returns it, otherwise, it returns unknown. **/
  Sign get_sign(int idx = 0) const; /**< returns the sign of one instance of the
                                       current parameter/variable. **/
  pair<double, double>* get_range() const;

  /** Operators */
  bool operator==(const param_& p) const {
    return (_unique_id == p._unique_id);
    //        return (_id==p._id && _type==p._type && _intype==p._intype &&
    //        get_name()==p.get_name());
  }
    ...
    if (_is_indexed) {
      if (_ids->size() > 1) {
        throw invalid_argument(
            "get_nb_instances should be called with index here\n");
      }
...
class param : public param_ {
 ...
 public:
  ...
  param() {
    ...
    //    throw invalid_argument("Please enter a name in the parameter
    //    constructor");
    update_type();
    ...
    _range = make_shared<pair<type, type>>(make_pair<>(
        numeric_limits<type>::max(), numeric_limits<type>::lowest()));
  }
  ...
  shared_ptr<vector<type>> get_vals() const { return _val; }
  ...
  type eval(unsigned i) const {
    if (_is_indexed) {
      //                if (_ids->size()>1) {
      //                    throw invalid_argument("eval() should be called with
      //                    double index here\n");
      //                }
  ...
  bool is_unit()
      const { /**< Returns true if all values of this paramter are 1 **/
    return (_range->first == 1 && _range->second == 1);
  }

  bool is_zero()
      const { /**< Returns true if all values of this paramter are 0 **/
  ...
  bool is_positive()
      const { /**< Returns true if all values of this paramter are positive **/
    return (_range->first > 0 && _range->second > 0);
  }
  ...
  bool operator==(const param& p) const {
    return (get_name() == p.get_name() && _type == p._type &&
            _intype == p._intype && _dim == p._dim && _indices == p._indices &&
            _sdpindices == p._sdpindices && _val == p._val);
    // return (get_name()==p.get_name() && _type==p._type && _intype==p._intype
    // && _dim==p._dim && _indices==p._indices && _val==p._val);
  }
  ...
  void set_vals(
      const Eigen::SparseMatrix<complex<double>, Eigen::RowMajor>& SM) {
    for (int k = 0; k < SM.outerSize(); ++k) {
      for (Eigen::SparseMatrix<complex<double>, Eigen::RowMajor>::InnerIterator
               it(SM, k);
           it; ++it) {
  ...
  void QuantumT(unsigned qubit_pos, unsigned nb_qubits,
                bool transpose = false) {
    using namespace qpp;
    if (transpose) {
      auto U = gt.expandout(adjoint(gt.T), qubit_pos, nb_qubits);
      Debug("T transpose matrix at position " << to_string(qubit_pos) << " = "
                                              << endl);
      Debug(disp(U) << "\n");
  ...
  /* Matrix representation of a Quantum Cnot gate with qc as control qubit and
   * qt as target one */
  void QuantumCnot(unsigned qc, unsigned qt, unsigned nb_qubits) {
    using namespace qpp;
    auto U = gt.expandout(gt.CTRL(gt.X, {qc}, {qt}, nb_qubits), 0, 1,
                          pow(2, nb_qubits));
    Debug("Cnot matrix from " << to_string(qc) << " to " << to_string(qt)
                              << " = " << endl);
    Debug(disp(U) << "\n");
    set_vals(U.sparseView());
    Debug(to_str(true));
  ...
  void initialize(size_t i, type v) { set_val(i, v); }
  ...
  //            res._rev_indices = this->_rev_indices; res._indices =
  //            this->_indices; res._ids->at(0).push_back(idx); res._dim[0]=1;
  //            res._name += "["+to_string(idx)+"]";
  //            res._unique_id =
  //            make_tuple<>(res._id,unindexed_,typeid(type).hash_code(), idx,
  //            1); res._is_indexed = true;
  ...
  param operator()(size_t t1, Args&&... args) {
    ...
    res._dim[0] = 1;
    res._name += "[" + key + "]";
    res._unique_id = make_tuple<>(res._id, unindexed_, typeid(type).hash_code(),
  ...
  param operator()(string t1, Args&&... args) {
    ...
    res._dim[0] = 1;
    res._name += "[" + key + "]";
    res._unique_id = make_tuple<>(
        res._id, unindexed_, typeid(type).hash_code(), res._ids->at(0).at(0),
  ...
  param in(const vector<Tobj>& vec) {
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.in(const vector<Tobj*>& vec), vec is empty!\n. "
    ...
    res._name += ".in_" + vec.front()._type_name + "," + excluded + "," +
                 to_string(res._dim[0]);
    res._unique_id =
        make_tuple<>(res._id, in_, typeid(Tobj).hash_code(), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param prev(const indices& vec1, Args&&... args) {
    ...
    if (vecs.size() == 1 && res._indices->size() == 0) {
      ...
      res._dim[0] = dim;
      res._name += ".prev";
      res._unique_id = make_tuple<>(res._id, prev_, typeid(indices).hash_code(),
                                    0, res._dim[0]);
      res._is_indexed = true;
      return res;
    }
  ...
  template <typename Tobj>
  param min_time(const vector<Tobj*>& vec, const indices& ids,
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.out_arcs(const vector<Tobj*>& vec), vec is "
          "empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      res._is_indexed = true;
      return res;
    ...
    for (auto it = vec.begin(); it != vec.end(); it++) {
      ...
      for (auto i = 0; i < ids.size(); i++) {
        ...
        for (auto j = i; j < min(i + time.eval((*it)->_name), (int)ids.size());
             j++) {
    ...
    res._unique_id = make_tuple<>(res._id, min_time_, typeid(Tobj).hash_code(),
                                  0, res._dim.size());
    res._is_indexed = true;
    return res;
  }

  int nthOccurrence(const std::string& str, const std::string& findMe,
                    int nth) {
    size_t pos = 0;
    int cnt = 0;
  ...
  param in(const indices& vec1, Args&&... args) {
    ...
    res._rev_indices = this->_rev_indices;
    res._indices = this->_indices;
    if (ids._excluded_indices.empty() &&
        res._time_extended == ids._time_extended &&
        (res._indices->size() == ids.size() || res._indices->size() == 0)) {
    ...
    if (res._indices->size() == 0) {
      ...
      res._dim[0] = dim;
      if (dim > 0) {
        res._name += ".in_indices(" + vec1._indices->at(0) + "," +
                     vec1._indices->at(vec1._indices->size() - 1) + ")";
      } else {
        res._name += ".empty_index_set";
      }
      res._unique_id =
          make_tuple<>(res._id, in_, typeid(indices).hash_code(), 0, dim);
      res._is_indexed = true;
      return res;
    }
    size_t idx = 0;
    for (auto key : *ids._indices) {
      ...
      if (res._indices->size() == 1 &&
          res._rev_indices->at(0).compare(key) != 0) {
        key = res._rev_indices->at(0);
      }
      if (ids._time_extended) {  // TODO is this if needed here?
        auto nb_sep1 = count(_rev_indices->front().begin(),
                             _rev_indices->front().end(), ',');
    ...
    res._unique_id = make_tuple<>(res._id, in_, typeid(indices).hash_code(), 0,
                                  res._dim[0] * res._dim.size());
    res._is_indexed = true;
    return res;
  }

  template <typename Tobj>
  param submat(const vector<Tobj>& vec) {
    param res(this->_name);
    res._id = this->_id;
    res._vec_id = this->_vec_id;
    res._intype = this->_intype;
  ...
  param in_arcs(const vector<Node*>& vec) {
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.in_arcs(const vector<Tobj*>& vec), vec is "
          "empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      res._is_indexed = true;
  ...
  param in_arcs_at(const vector<Node*>& vec, const int t) {
    param res(this->_name);
    res._id = this->_id;
    res._vec_id = this->_vec_id;
    res._intype = this->_intype;
  ...
  param out_arcs(const vector<Node*>& vec) {
    ...
    res._unique_id = make_tuple<>(res._id, out_arcs_, typeid(Node).hash_code(),
                                  0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param out_arcs_at(const vector<Node*>& vec, const int t) {
    param res(this->_name);
    res._id = this->_id;
    res._vec_id = this->_vec_id;
    res._intype = this->_intype;
  ...
  param in_gens(const vector<Node*>& vec) {
    ...
    res._unique_id = make_tuple<>(res._id, in_gens_, typeid(Node).hash_code(),
                                  0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  param in_gens(const vector<Node*>& vec, const unsigned T) {
    ...
    // res.print(true);
    res._unique_id = make_tuple<>(res.get_id(), in_gens_time_,
                                  typeid(Node).hash_code(), 0, 0);
    res._is_indexed = true;
    return res;
  }
  param in_gens_at(const vector<Node*>& vec, const unsigned t) {
    ...
    res._unique_id =
        make_tuple<>(res._id, in_gens_at_, typeid(Node).hash_code(), 0, 0);
    res._is_indexed = true;
    return res;
  }
  ...
  param in_pot_gens(const vector<Node*>& vec) {
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.in_pot_gens(const vector<Tobj*>& vec), vec is "
          "empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      res._is_indexed = true;
    ...
    res._unique_id = make_tuple<>(res._id, in_pot_gens_,
                                  typeid(Node).hash_code(), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param in_pot_bats(const vector<Node*>& vec) {
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.in_pot_bats(const vector<Tobj*>& vec), vec is "
          "empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      res._is_indexed = true;
    ...
    res._unique_id = make_tuple<>(res._id, in_pot_bats_,
                                  typeid(Node).hash_code(), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param in_bats(
      ...
      const indices& T) {  // TODO MERGE THIS WITH IN GENS AND OTHER FUNCS
    param res(this->_name);
    res._id = this->_id;
    res._vec_id = this->_vec_id;
    res._intype = this->_intype;
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.in_bats(const vector<Tobj*>& vec, const indices& "
          "T), vec is empty!\n. Creating and empty variable! Check your "
          "sum/product operators.\n");
      res._name += "EMPTY_VAR";
      res._is_indexed = true;
      return res;
    }
    ...
    res._unique_id = make_tuple<>(res._id, in_bats_, typeid(Node).hash_code(),
                                  0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param in_pv(
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.in_pv(const vector<Tobj*>& vec, const indices& "
          "T), vec is empty!\n. Creating and empty variable! Check your "
          "sum/product operators.\n");
      res._name += "EMPTY_VAR";
      res._is_indexed = true;
    ...
    res._unique_id =
        make_tuple<>(res._id, in_pv_, typeid(Node).hash_code(), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param in_wind(
    ...
    if (vec.empty()) {
      DebugOn(
          "In function param.in_wind(const vector<Tobj*>& vec, const indices& "
          "T), vec is empty!\n. Creating and empty variable! Check your "
          "sum/product operators.\n");
      res._name += "EMPTY_VAR";
      res._is_indexed = true;
    ...
    res._unique_id = make_tuple<>(res._id, in_wind_, typeid(Node).hash_code(),
                                  0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param in_pairs(const vector<Tobj*>& vec) {
    ...
    if (vec.empty()) {
      DebugOff(
          "In function param.in_pairs(const vector<Tobj*>& vec), vec is "
          "empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      return res;
  ...
  template <typename Tobj>
  param in_pairs(const vector<Tobj*>& vec, const indices& ids) {
    param res(this->_name);
    res._id = this->_id;
    res._vec_id = this->_vec_id;
    res._intype = this->_intype;
  ...
  param in_pairs(const vector<Tobj*>& vec, unsigned T) {
    ...
    if (vec.empty()) {
      DebugOff(
          "In function param.in_pairs(const vector<Tobj*>& vec, unsigned T), "
          "vec is empty!\n. Creating and empty variable! Check your "
          "sum/product operators.\n");
      res._name += "EMPTY_VAR";
      return res;
  ...
  template <typename Tobj>
  param in_pairs_at(const vector<Tobj*>& vec, const unsigned t) {
    param res(this->_name);
    string key;
    vector<string> keys;
    if (vec.empty()) {
    ...
    res._unique_id = make_tuple<>(res.get_id(), in_pairs_at_,
                                  typeid(Tobj).hash_code(), 0, 0);
    return res;
  }
  ...
  param from(const vector<Tobj*>& vec) {
    ...
    if (vec.empty()) {
      DebugOff(
          "In function param.from(const vector<Tobj*>& vec), vec is empty!\n. "
          "Creating and empty variable! Check your sum/product operators.\n");
      res._name += "EMPTY_VAR";
      return res;
    }
    ...
    res._unique_id =
        make_tuple<>(res._id, from_, typeid(Tobj).hash_code(), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  template <typename Tobj>
  param from(const vector<Tobj*>& vec, const indices& ids) {
    param res(this->_name);
    res._id = this->_id;
    res._vec_id = this->_vec_id;
    res._intype = this->_intype;
  ...
  param to(const vector<Tobj*>& vec) {
    ...
    if (vec.empty()) {
      DebugOff(
          "In function param.to(const vector<Tobj*>& vec), vec is empty!\n. "
          "Creating and empty variable! Check your sum/product operators.\n");
      res._name += "EMPTY_VAR";
      return res;
    }
    ...
    res._name += "_" + vec.front()->_type_name;
    res._unique_id =
        make_tuple<>(res._id, to_, typeid(Tobj).hash_code(), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }

  template <typename Tobj>
  param to(const vector<Tobj*>& vec, const indices& ids) {
    param res(this->_name);
    res._id = this->_id;
    res._vec_id = this->_vec_id;
    res._intype = this->_intype;
  ...
  template <typename Tobj>
  param to_at(const vector<Tobj*>& vec, const unsigned t) {
    param res(this->_name);
    string key;
    vector<string> keys;
    for (auto it = vec.begin(); it != vec.end(); it++) {
    ...
    res._unique_id =
        make_tuple<>(res.get_id(), to_at_, typeid(Tobj).hash_code(), 0, 0);
    return res;
  }
  ...
  param excl(unsigned index) {
    ...
    res._name += ".excl(" + to_string(index) +
                 ")";  // _name and _unique_id should be doublely unique.
    res._unique_id =
        make_tuple<>(res._id, in_, typeid(type).hash_code(), index, index);
    res._is_indexed = true;
    return res;
  }
  ...
  param in(const Tobj nm, unsigned T) {
    ...
    res._dim[0] = res._ids->at(0).size();
    res._name +=
        ".in_time_" + nm->_name + "_time_" +
        to_string(T);  // _name and _unique_id should be doublely unique.
    hash<string> str_hash;
    res._unique_id =
        make_tuple<>(res._id, in_time_, str_hash(nm->_name), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param in_at(const vector<Tobj*>& nodes, unsigned t) {
    ...
    if (nodes.empty()) {
      DebugOff(
          "In function in_at(const vector<Tobj*>& nodes, unsigned t), nodes is "
          "empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      return res;
    ...
    res._unique_id =
        make_tuple<>(res._id, in_at_, typeid(Tobj).hash_code(), 0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param in(const vector<Tobj*>& nodes, unsigned T) {
    ...
    if (nodes.empty()) {
      DebugOff(
          "In function param.in(const vector<Tobj*>& nodes, unsigned T), nodes "
          "is empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      return res;
  ...
  param from(const vector<Tobj*>& arcs, unsigned T) {
    ...
    if (arcs.empty()) {
      DebugOff(
          "In function param.from(const vector<Tobj*>& arcs, unsigned T), arcs "
          "is empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      return res;
    ...
    res._dim[0] = res._ids->at(0).size();
    res._name +=
        ".from_" + string(typeid(Tobj).name()) + "_time_" + to_string(T);
    res._unique_id = make_tuple<>(res._id, from_time_, typeid(Tobj).hash_code(),
                                  0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  param to(const vector<Tobj*>& arcs, unsigned T) {
    ...
    if (arcs.empty()) {
      DebugOff(
          "In function param.to(const vector<Tobj*>& arcs, unsigned T), arcs "
          "is empty!\n. Creating and empty variable! Check your sum/product "
          "operators.\n");
      res._name += "EMPTY_VAR";
      return res;
    ...
    res._unique_id = make_tuple<>(res._id, to_time_, typeid(Tobj).hash_code(),
                                  0, res._dim[0]);
    res._is_indexed = true;
    return res;
  }
  ...
  void time_expand(const indices& T) {
    ...
    set_size(dim);
    //        for (map<std::string, unsigned>::iterator it=
    //        param::_indices->begin(); it != param::_indices->end(); it++){
    //            key = it->first;
    ...
    _name += ".time_expanded";
    _unique_id = make_tuple<>(_id, unindexed_, typeid(type).hash_code(), 0,
                              dim * T.size());
  }
  ...
  param time_collapse(const indices& T) {
    ...
    res._unique_id =
        make_tuple<>(get_id(), unindexed_, typeid(type).hash_code(), 0, dim);
    return res;
  }
  ...
  void time_expand(unsigned T) {
    ...
    _name += ".time_expanded";
    _unique_id =
        make_tuple<>(_id, in_time_, typeid(type).hash_code(), 0, dim * T);
  }
  ...
  void print(size_t index) const { cout << to_str(index); }
    ...
    if (vals) {
      ...
      if (_rev_indices->size() > 0) {
        for (unsigned idx = 0; idx < _val->size(); idx++) {
          //                        if (!_is_relaxed ||
          //                        fabs(roundf(_val->at(idx)) - _val->at(idx))
          //                        > 1e-4) {
          str += name + "(" + _rev_indices->at(idx) +
                 ")=" + to_string_with_precision(_val->at(idx), 10);
          str += " \n";
          //                        }
        }
      } else {
        for (auto& indsp : *_indices) {
          str += name + "(" + indsp.first + "=" +
                 to_string_with_precision(_val->at(indsp.second), 10) + ")";
          str += " ";
        }
      }
      str += "];\n";
  ...
  void print(bool vals = false) const { cout << this->to_str(vals); }
