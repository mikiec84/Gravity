...
class Constraint : public func_ {
 ...
 public:
  unsigned _jac_cstr_idx; /* Firt index of the corresponding non-zero values in
                             the Jacobian */
  unsigned _id = 0;
  ...
  Constraint& in(const vector<Tobj*>& vec, const indices& ind) {
  ...
  template <typename Tobj>
  Constraint& in(const vector<Tobj*>& vec) {
    this->func_::in(vec);
    return *this;
  };
