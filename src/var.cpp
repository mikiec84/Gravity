...
var<type>::var() : param<type>() {
  param<type>::set_type(var_c);
};

template <typename type>
var<type>::var(const string& name) : param<type>(name) {
  param<type>::set_type(var_c);
  if (this->is_binary()) {
...
template <typename type>
var<type>::var(const string& name, Sign s) : param<type>(name) {
  param<type>::set_type(var_c);
...
template <typename type>
var<type>::var(const var<type>& v) : param<type>(v) {
  param<type>::set_type(var_c);
  _lb = v._lb;
  _ub = v._ub;
...
template <typename type>
var<type>::var(var<type>&& v) : param<type>(v) {
  param<type>::set_type(var_c);
  _lb = move(v._lb);
  _ub = move(v._ub);
  param<type>::_is_relaxed = v._is_relaxed;
...
template <typename type>
var<type>::var(const string& name, type lb, type ub) : var(name) {
...
template <typename type>
var<type>::var(const string& name, func_&& lb, func_&& ub) : var(name) {
  _lb = make_shared<func_>(move(lb));
  _ub = make_shared<func_>(move(ub));
}

template <typename type>
var<type>::var(const string& name, const func_& lb, const func_& ub)
    : var(name) {
  _lb = make_shared<func_>(lb);
  _ub = make_shared<func_>(ub);
}

// template<typename type> var<type>::var(const string& name, const param<type>&
// lb, const param<type>& ub):var(name) {
//    _lb = make_shared<func_>(lb);
//    _ub = make_shared<func_>(ub);
...
// template<typename type> var<type>::var(const string& name, func_&& lb,
// func_&& ub):var(name) {
//    _lb = make_shared<func_>(move(lb));
//    _ub = make_shared<func_>(move(ub));
...
template <typename type>
var<type>::var(const string& name, const param<type>& sb) : var(name) {
  ...
  _lb->_val->resize(nb_inst);
  _ub->_val->resize(nb_inst);
  param<type>::_range->first =
      min(-1 * sb._range->first, -1 * sb._range->second);
...
//    template<typename type> var<type>::var(const string& name, func_&&
//    sb):var(name) {
//        _lb = make_shared<func_>(sb*-1);
//        _ub = make_shared<func_>(move(sb));
//        param<type>::_range->first =
//        min(-1*sb.get_all_range()->first,-1*sb.get_all_range()->second);
//        param<type>::_range->second = sb.get_all_range()->second;
//        param<type>::_ids = sb._ids;
//    };

template <typename type>
var<type>& var<type>::operator=(const var<type>& v) {
...
template <typename type>
var<type>& var<type>::operator=(var<type>&& v) {
...
template <typename type>
void var<type>::set_size(vector<size_t> dims) {
  param<type>::set_size(dims);
};

template <typename type>
void var<type>::set_size(size_t s, type val) {
...
template <typename type>
void var<type>::add_lb_only(type val) {
...
template <typename type>
void var<type>::add_ub_only(type val) {
...
template <typename type>
void var<type>::set_lb(int i, type val) {
  *_lb = constant<type>(val);
  param<type>::_range->first = val;
}

template <typename type>
void var<type>::set_ub(int i, type val) {
  *_ub = constant<type>(val);
  param<type>::_range->second = val;
}

template <typename type>
void var<type>::in_q_cone() {
  _in_q_cone = true;
};
...
/* Operators */
template <typename type>
bool var<type>::operator==(const var& v) const {
...
/* Output */
template <typename type>
void var<type>::print(bool bounds) const {
...
template <typename type>
sdpvar<type>::sdpvar() : var<type>() {
  var<type>::set_type(sdpvar_c);
};

template <typename type>
sdpvar<type>::sdpvar(const string& name) : var<type>(name) {
  var<type>::set_type(sdpvar_c);
};

template <typename type>
sdpvar<type>::sdpvar(const sdpvar<type>& v) : var<type>(v) {
  var<type>::set_type(sdpvar_c);
  _symdim = v._symdim;
};

template <typename type>
sdpvar<type>::sdpvar(sdpvar<type>&& v) : var<type>(v) {
  var<type>::set_type(sdpvar_c);
  _symdim = v._symdim;
};
...
// sdpvar
template <typename type>
sdpvar<type>::sdpvar(const string& name, type lb, type ub)
    : sdpvar<type>(name) {
  var<type>(name, lb, ub);
};
...
// Operators
template <typename type>
bool sdpvar<type>::operator==(const sdpvar& v) const {
  return (param<type>::operator==(v));
};
...
/* output */
template <typename type>
void sdpvar<type>::print(bool vals) const {
  param<type>::print(vals);
};

template <typename type>
vector<var<type>> var<type>::pairs_in_directed(
    Net& net, const std::vector<std::vector<Node*>>& bags, unsigned size) {
  vector<var> res;
  string key;
  res.resize(size);
  for (int i = 0; i < size; i++) {
    ...
    res[i]._unique_id =
        make_tuple<>(res[i]._id, in_, typeid(type).hash_code(), 0, i);
  ...
  for (auto& bag : bags) {
    ...
    for (int i = 0; i < size - 1; i++) {
      ...
      if (pp.second) {  // new index inserted
        ...
        param_::_rev_indices->resize(
            max(param_::_rev_indices->size(), index + 1));
...
vector<var<type>> var<type>::pairs_in_directed(
  ...
  for (int i = 0; i < size; i++) {
    ...
    res[i]._unique_id =
        make_tuple<>(res[i].get_id(), in_, typeid(type).hash_code(), 0, i);
    res[i]._is_indexed = true;
    res[i]._rev_indices = this->_rev_indices;
    res[i]._indices = this->_indices;
  }
...
template <typename type>
vector<var<type>> var<type>::pairs_in(
    const std::vector<std::vector<Node*>>& bags, unsigned size) {
  vector<var> res;
  string key;
  ...
  for (auto& bag : bags) {
    ...
    for (int i = 0; i < size - 1; i++) {
      ...
      if (pp.second) {  // new index inserted
        ...
        if (res[i]
                ._indices->insert(make_pair<>(key, pp.first->second))
                .second) {
          res[i]._dim[0]++;
        }
        res[i]._ids->at(0).push_back(pp.first->second);
...
template <typename type>
vector<var<type>> var<type>::in(const std::vector<std::vector<Node*>>& bags,
                                unsigned size) {
  vector<var> res;
  string key;
  res.resize(size);
...
template <typename type>
vector<var<type>> var<type>::in_at(const std::vector<std::vector<Node*>>& bags,
                                   unsigned size, int t) {
  vector<var> res;
  string key;
  res.resize(size);
