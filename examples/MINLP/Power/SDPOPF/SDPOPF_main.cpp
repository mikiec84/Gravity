...
int main (int argc, char * argv[]) {
    ...
    opt.add_option("a", "algorithm", "Algorithm: LPC(Linear & Polynomial Cuts)/LL(Cholesky reformulation), default = LPC", algo_str);
    ...
    if (algo_str.compare("LL")==0) {
        ...
        var<Real>  R_Lij("R_Lij");
        /* Imaginary part of Wij = ViVj */
        var<Real>  Im_Wij("Im_Wij", grid.wi_min, grid.wi_max);
        var<Real>  Im_Lij("Im_Lij");
        /* Magnitude of Wii = Vi^2 */
        var<Real>  Wii("Wii", grid.w_min, grid.w_max);
        var<Real>  Lii("Lii", pos_);
        SDP.add_var(Wii.in(grid.nodes));
        SDP.add_var(Lii.in(grid._bags,grid.nodes));
        SDP.add_var(R_Wij.in(bus_pairs_chord));
        SDP.add_var(Im_Wij.in(bus_pairs_chord));
        SDP.add_var(R_Lij.in(grid._bags,bus_pairs_chord));
        SDP.add_var(Im_Lij.in(grid._bags,bus_pairs_chord));
        
        /* Initialize variables */
        R_Wij.initialize_all(1.0);
        R_Lij.initialize_all(1.0);
        Wii.initialize_all(1.001);
        Lii.initialize_all(1.001);
        ...
        size_t b_size = 3;
        auto R_Wij_ = R_Wij.pairs_in_directed(grid, grid._bags, b_size);
        auto Im_Wij_ = Im_Wij.pairs_in_directed(grid, grid._bags, b_size);
        auto Wii_ = Wii.in(grid._bags, b_size);
        auto R_Lij_ = R_Lij.pairs_in_directed(grid, grid._bags, b_size, true);
        auto Im_Lij_ = Im_Lij.pairs_in_directed(grid, grid._bags, b_size, true);
        auto Lii_ = Lii.in(grid._bags, b_size, true);
        auto I_sgn = signs(grid, grid._bags);
        for (int i = b_size-1; i >= 0; i--) {
            /* Diagonal elements */
            Constraint W("W_"+to_string(i));
            W = Wii_[i] - Lii_[i]*Lii_[i];
            for (int k = b_size-1; k>i; k--) {
                W -= R_Lij_[id(i,k)]*R_Lij_[id(i,k)] + Im_Lij_[id(i,k)]*Im_Lij_[id(i,k)];
            }
            SDP.add(W == 0);
            W.print_expanded();
            
            for (int j = b_size-1; j>i; j--) {
                /* Real part of non-diagonal elements*/
                Constraint R_W("R_W_"+to_string(i)+to_string(j));
                R_W = R_Wij_[id(i,j)] - R_Lij_[id(i,j)]*Lii_[j];
                for (int k = b_size-1; k>j; k--) {
                    R_W -= R_Lij_[id(i,k)]*R_Lij_[id(j,k)] + I_sgn[id(i,k)]*I_sgn[id(j,k)]*Im_Lij_[id(i,k)]*Im_Lij_[id(j,k)];
                }
                SDP.add(R_W == 0);
                R_W.print_expanded();
                /* Imaginary part of non-diagonal elements*/
                Constraint I_W("I_W_"+to_string(i)+to_string(j));
                I_W = Im_Wij_[id(i,j)] - I_sgn[id(i,j)]*Im_Lij_[id(i,j)]*Lii_[j];
                for (int k = b_size-1; k>j; k--) {
                    I_W -= I_sgn[id(i,k)]*Im_Lij_[id(i,k)]*R_Lij_[id(j,k)] - I_sgn[id(j,k)]*R_Lij_[id(i,k)]*Im_Lij_[id(j,k)];
                }
                SDP.add(I_W == 0);
                I_W.print_expanded();
            }
        ...
        //    Constraint Wii_0("Wii_0");
        //    Wii_0 = Wii_[0] - Lii_[0]*Lii_[0];
        //    SDP.add(Wii_0 == 0);
        //    Constraint Wii_1("Wii_1");
        //    Wii_1 = Wii_[1] - Lii_[1]*Lii_[1] - R_Lij_[0]*R_Lij_[0] - Im_Lij_[0]*Im_Lij_[0];
        //    SDP.add(Wii_1 == 0);
        //    Constraint Wii_2("Wii_2");
        //    Wii_2 = Wii_[2] - Lii_[2]*Lii_[2] - R_Lij_[0]*R_Lij_[0] - Im_Lij_[0]*Im_Lij_[0] - R_Lij_[1]*R_Lij_[1] - Im_Lij_[1]*Im_Lij_[1];
        //    SDP.add(Wii_2 == 0);
...
//        SDPOPF.run(output = 5, relax = false);
        SDPOPF.run(output, relax = false, tol, "ma27", mehrotra = "no");
        ...
        while(cuts_added > 0 && gap > 1) {
            ...
            SDPOPF.run(output = 5, relax = false, tol, "ma27", mehrotra = "no");
