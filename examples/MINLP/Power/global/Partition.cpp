...
void Partition::get_ncut(const PowerNet& grid, const unsigned& nbparts) {
  ...
  for (auto& arc : grid.arcs) {
    adjacency_matrix(arc->_src->_id, arc->_dest->_id) =
        1.0;  // += generalises the case of parallel edges, but it is not
              // necessarily better.
  ...
  // W = adjacency_matrix + arma::spdiags(dr,0,grid.nodes.size(),
  // grid.nodes.size());// sparsified form. regularisation
  double eps = 0.0;
  ...
  for (int i = 0; i < arma::size(eigenvectors, 1); i++) {
    ...
    eigenvectors.col(i) =
        (eigenvectors.col(i) / arma::norm(eigenvectors.col(i))) *
        arma::norm(arma::ones(grid.nodes.size(), 1));
    if (eigenvectors(0, i) != 0) {
      int l = (0 < eigenvectors(0, i)) - (eigenvectors(0, i) < 0);
      if (l > 0) eigenvectors.col(i) *= -1;
    }
  }
  ...
  double pos =
      rand() % grid.nodes.size();  // random number between 0 to size-1.
  ...
  while (exitLoop == 0) {
    ...
    // J is the index of the maxima of each row corresponding to grid.nodes.size
    // x nparts matrix So it is grid.nodes.size x 1
  ...
  // Generate a graph to represent the resulting partition where an edge is
  // formed between two nodes that induces cuts.
  ...
  for (auto arc : grid.arcs) {
    ...
    if (from == to) {
      ...
      // GRAPH IS DIRECTED.. used to partition the bus pairs (the total linking
      // constraints is the same regardless of the type of the G_part..
      // bus_pairs are also directed. )
      if (ac != nullptr) {
        string key = arc->_src->_name + "," + arc->_dest->_name;
        string key_inv = arc->_dest->_name + "," + arc->_src->_name;
        if (indexpair1.find(key) == indexpair1.end() &&
            indexpair1.find(key_inv) == indexpair1.end()) {
          indexpair1.insert(key);
          ac->_intersection_clique.push_back(new index_pair(
              index_(arc->_src->_name), index_(arc->_dest->_name), true));
  ...
  for (int c = 0; c < nbparts; c++) {
    ...
    for (auto a : bag_arcs_disjoint[c]) {
      ...
      if (indexij.find(key) == indexij.end() &&
          indexij.find(key_inv) == indexij.end()) {
        pair.push_back(new index_pair(index_(a->_src->_name),
                                      index_(a->_dest->_name), true));
  ...
  for (auto b : G_part.arcs) {
    for (auto a : b->_intersection_clique) {
      string key = a->_src->_name + "," + a->_dest->_name;
      string key_inv = a->_dest->_name + "," + a->_src->_name;
      if (indexpair.find(key) == indexpair.end() &&
          indexpair.find(key_inv) == indexpair.end()) {
        ...
        // neighbouring pairs to two parts), the source bag must contain the
        // source nodes of the arcs.
  ...
  for (int c = 0; c < nbparts; c++) {
    ...
    test.insert(test.end(), bag_bus_pairs_disjoint[c].begin(),
                bag_bus_pairs_disjoint[c].end());
    test.insert(test.end(), bag_bus_pairs_neighbour[c].begin(),
                bag_bus_pairs_neighbour[c].end());
    pairs.insert(pairs.end(), bag_bus_pairs_disjoint[c].begin(),
                 bag_bus_pairs_disjoint[c].end());
    pairs.insert(pairs.end(), bag_bus_pairs_neighbour_directed[c].begin(),
                 bag_bus_pairs_neighbour_directed[c].end());
    temp.insert(temp.end(), bag_arcs_disjoint[c].begin(),
                bag_arcs_disjoint[c].end());
    temp.insert(temp.end(), bag_arcs_neighbour[c].begin(),
                bag_arcs_neighbour[c].end());
  ...
  for (auto i = 0; i < nbparts; ++i) {
    vector<Line*> temp;
    vector<Line*> temp1;
    temp.insert(temp.end(), bag_arcs_disjoint[i].begin(),
                bag_arcs_disjoint[i].end());
    temp.insert(temp.end(), bag_arcs_out[i].begin(), bag_arcs_out[i].end());
    temp1.insert(temp1.end(), bag_arcs_disjoint[i].begin(),
                 bag_arcs_disjoint[i].end());
